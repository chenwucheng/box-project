...
router.post('/',async (ctx, next) => {
    let {user,fl,priced,price,kq,zt,addtime} = ctx.request.body;       
    
    let res = await db.insert('goodslist',{user,fl,priced,price,kq,zt,addtime})
...
router.get('/',async (ctx, next) => {
    ...
    let res = await db.find('goodslist', { user });
